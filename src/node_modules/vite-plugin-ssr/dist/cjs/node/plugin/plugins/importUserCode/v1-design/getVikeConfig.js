...
async function loadVikeConfig(userRootDir, isDev, extensions) {
    ...
    const pageConfigs = await Promise.all(Object.entries(interfaceFilesByLocationId)
        ...
        const { routeFilesystem, routeFilesystemDefinedBy, isErrorPage } = determineRouteFilesystem(locationId, configValueSources);
        const pageConfig = {
            pageId: locationId,
            isErrorPage,
...
function getConfigValueSource(configName, interfaceFile, configDef, userRootDir) {
    ...
    if (configDef._valueIsFilePath) {
        ...
        if (interfaceFile.isConfigFile) {
            ...
            (0, utils_js_1.assertUsage)(codeFile, `${configDefinedAt} should be an import`);
...
function assertCodeFileEnv(codeFilePath, configEnv, configName) {
...
function resolveRelativeCodeFilePath(importData, configFilePath, userRootDir) {
    ...
    (0, utils_js_1.assertPosixPath)(codeFilePath);
    ...
    (0, utils_js_1.assertPosixPath)(codeFilePath);
    (0, utils_js_1.assert)(codeFilePath.startsWith('/'));
    return codeFilePath;
}
...
function determineRouteFilesystem(locationId, configValueSources) {
    ...
    let routeFilesystem = (0, filesystemRouting_js_1.getRouteFilesystem)(locationId);
    if (determineIsErrorPage(routeFilesystem)) {
        return { isErrorPage: true, routeFilesystem: null, routeFilesystemDefinedBy: null };
    }
    let routeFilesystemDefinedBy = (0, filesystemRouting_js_1.getRouteFilesystemDefinedBy)(locationId); // for log404()
    if (configFilesystemRoutingRoot) {
        ...
        if (routingRoot) {
            ...
            (0, utils_js_1.assert)(routeFilesystem.startsWith(filesystemRoutingRootEffect.before), debugInfo);
            routeFilesystem = (0, filesystemRouting_js_1.applyFilesystemRoutingRootEffect)(routeFilesystem, filesystemRoutingRootEffect);
            (0, utils_js_1.assert)(filesystemRoutingRootDefinedAt.includes('export'));
            routeFilesystemDefinedBy = `${routeFilesystemDefinedBy} (with ${filesystemRoutingRootDefinedAt})`;
        }
    }
    (0, utils_js_1.assert)(routeFilesystem.startsWith('/'));
...
function getFilesystemRoutingRootEffect(configFilesystemRoutingRoot, configName) {
    ...
    const configDefinedAt = (0, utils_js_2.getConfigDefinedAtString)(configName, configFilesystemRoutingRoot, true);
    ...
    const before = (0, filesystemRouting_js_1.getRouteFilesystem)((0, filesystemRouting_js_1.getLocationId)(configFilesystemRoutingRoot.definedAtInfo.filePath));
    const after = value;
