...
async function getEntries(config) {
    ...
    assertUsage(Object.keys(pageFileEntries).length !== 0 || pageConfigsData.length !== 0, 'At least one page should be defined, see https://vite-plugin-ssr.com/add');
    if (viteIsSSR(config)) {
...
function assertRollupInput(config) {
    ...
    assertUsage(htmlInput === undefined, `The entry ${htmlInput} of config build.rollupOptions.input is an HTML entry which is forbidden when using vite-plugin-ssr, instead follow https://vite-plugin-ssr.com/add`);
}
