...
function assertResolveAlias(config) {
    ...
    aliases.forEach((alias) => {
        ...
        assertWarning(customResolver === undefined, `${errPrefix} defines resolve.alias with customResolver() ${errSuffix2}`, { onlyOnce: true });
        if (typeof find !== 'string') {
            ...
            if (find.startsWith('react'))
                return;
            assertWarning(isValidPathAlias(find), `${errPrefix} defines an alias ${pc.cyan(find)} that cannot be distinguished from npm package imports, ${errSuffix1}`, { onlyOnce: true });
        }
    });
}
